## Standardise the units of measurements
## research alternative model fitting techniques
## , interactions, bootstraping, writing
...
# For the sake of consistency, covariables are kept if one of the levels has a significant p-value. 
...
# library(SignifReg)
# #scope1 <- wt~gestation+mparity+mht+drace+ded+dwt+msmoke+mtime+mnumber+mwt+marital+med+mage+inc+dage
# scope <- wt~.
# test.4 <- SignifReg(scope = scope, data = lmModel.1, alpha = 0.05, direction = "backward", criterion = "p-value")
# 
# test.5 <- SignifReg(scope = scope, data = lmModel.1, alpha = 0.05, direction = "step_null", criterion = "AIC")
# test.5
# 
# test.6 <- SignifReg(scope = scope, data = lmModel.1, alpha = 0.05, direction = "backward", criterion = "r-adj")
# test.6
# 
# test.7 <- SignifReg(scope = scope, data = lmModel.1, alpha = 0.05, direction = "step_null", criterion = "BIC")
# test.7
